...
class ReviewViewset(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if food_item_id and food_item_id not in ['null', 'undefined']:
            queryset = queryset.filter(food_item_id=food_item_id)
        return queryset
    
    def create(self, request, *args, **kwargs):
        if not request.user.is_authenticated:
            return Response({'error': 'You must be logged in to submit a review.'}, status=status.HTTP_401_UNAUTHORIZED)
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            if user:
                ...
                login(request, user)
                return Response({'token' : token.key, 'user_id' : user.id, 'is_admin': user.is_staff})
            else:
